...
(define-private (record-transfer (asset-id uint) (from principal) (to principal) (price (optional uint)))
  (let (
    (tx-id (unwrap-panic (get-txid)))
    (block-height block-height)
    (current-history (default-to {history: (list)} (map-get? asset-transfers {asset-id: asset-id})))
    (new-transfer {
      from: from,
      to: to,
      price: price,
    ...
    (map-set asset-transfers 
      {asset-id: asset-id} 
      {history: (if (> (len updated-history) u10)
                   (unwrap-panic (as-max-len? updated-history u10))
                   updated-history)}))
)

;; Calculate and distribute royalty payment
(define-private (handle-royalty (asset-id uint) (sale-price uint))
  (let (
...
(define-read-only (get-asset-history (asset-id uint))
  (default-to {history: (list)} (map-get? asset-transfers {asset-id: asset-id}))
)
...
(define-public (register-asset 
  ...
  (let (
    ...
    (asserts! (<= royalty-percentage u500) ERR-INVALID-ROYALTY) ;; Max 50% royalty
...
(define-public (cancel-asset-listing (asset-id uint))
  (let ((asset-data (unwrap! (map-get? assets {asset-id: asset-id}) ERR-ASSET-NOT-FOUND)))
    ...
    (asserts! (is-some (map-get? asset-listings {asset-id: asset-id})) ERR-NO-LISTING)
...
(define-public (update-asset-settings (asset-id uint) (is-transferable bool) (royalty-percentage uint))
  (let ((asset-data (unwrap! (map-get? assets {asset-id: asset-id}) ERR-ASSET-NOT-FOUND)))
    ;; Verify ownership
    (asserts! (is-eq tx-sender (get owner asset-data)) ERR-NOT-OWNER)
...
(define-read-only (verify-ownership (asset-id uint) (user principal))
  (match (map-get? assets {asset-id: asset-id})
